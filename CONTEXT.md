...
**Tier**:
A named position on the spectrum, defined by what the human stops doing and what engineered
verification replaces them with. Non-evaluative — a higher tier is not a better tier.
_Avoid_: framework (means React/Django to this audience), level (collides with autonomy levels,
which have no standard scale), stage, maturity level

**Spectrum**:
The ordered set of tiers, from minimal AI assistance to unsupervised autonomous delivery. Ordered
by degree of delegation only, not by desirability.
_Avoid_: maturity model, adoption curve, ladder (all imply higher is better)

**Posture**:
The set of beliefs and constraints — about trust, risk and what AI should touch — that lands a
team in a particular tier.
...
The current name for the practice this project's upper tiers describe.
